...
| `mt-creative-yonetmeni` | "reklam görseli", "ad creative", "creative üret", "reklam videosu", "ad video", "Reels video", "TikTok ad video", "image varyantı", "A/B test creative", "creative varyant", "ad copy yaz", "hook yaz", "headline yaz", "creative revizyon", "Shotstack outro", "Shotstack caption", "logo overlay", "paywall hero", "paywall görsel", "bulk varyant", "FLUX", "Kling video üret", "Seedance video", "Ideogram banner", "Nano Banana", "fal model", "creative üretim" |
| `mt-strateji-uzmani` | "bütçe nasıl", "bütçe dağılımı", "aylık plan", "üç aylık plan", "marketing stratejisi", "KPI", "hedef belirle", "allocation", "hedef ROAS", "target CPI", "target CPA", "payback period", "ayın bütçesi", "ay planı", "rebalance", "bütçe kaydır", "hedef install", "hedef gelir" |
| `mt-kampanya-analisti` | "kampanyalar nasıl", "performans nasıl", "haftalık rapor", "ROAS hesapla", "ROAS düştü", "LTV hesapla", "kes mi devam mı", "scale edebilir miyim", "cohort analizi", "raporlama", "haftayı kapat", "ay sonu retro", "retention düşük", "spike", "drop", "anomali", "CPI yükseldi", "CPA yükseldi" |
| `mt-rakip-arastirmaci` | "rakip", "rakip ne yapıyor", "rakip analiz", "rakip incele", "X uygulamasını analiz", "rakip paywall", "rakip pricing", "rakip ASO", "rakip creative", "competitor research", "ad library tara", "Meta Ad Library", "TikTok Creative Center", "Google Ads Transparency", "kategori benchmark", "kategori karşılaştır", "category analysis" |
| `mt-aso-uzmani` | "ASO", "App Store Optimization", "ASO audit", "ASO denetim", "ASO skoru", "keyword research", "App Store keyword", "keywords field", "screenshot optimize", "screenshot başlık", "App Store screenshot tasarım", "title subtitle", "store listing", "App Store description", "lokalizasyon", "App Store preview video", "CPP", "Custom Product Page yarat", "rating stratejisi", "review reply", "App Store ikon" |
| `mt-entegrasyon-kurucu` | "API bağla", "API kurulum", "token al", "API token", "Meta Marketing API", "TikTok Business API", "ASA Campaign Management API", "Google Ads API", "developer token", "OAuth flow", "JWT token", "refresh token", "MCP kur", "MCP ekle", "yeni entegrasyon", "programatik erişim", "developer account", "token expired", "401 hatası", "MCP failed to connect", "callback URL", "Firebase event handoff", "API entegrasyon" |
...
| "Reklam görseli" | Paid kampanya (brand-produced veya AI-UGC) → `mt-creative-yonetmeni` | (UGC creator outreach kapsam dışı — kullanıcı sadece AI üretiyor) |
...
| "Hook" / "copy" / "ad copy" | Paid reklam metni → `mt-creative-yonetmeni` | (kapsam dışı: bu sistem paid odaklı) |
...
| "TikTok video / creative" | Brand-produced veya AI-UGC tarzı üretim → `mt-creative-yonetmeni` | Spark Code teknik bağlama (kullanıcı bir post bulduysa) → `mt-tiktok-ads-uzmani` |
| "Creator" / "UGC creator" / "creator bulma" | **Kapsam dışı** — bu sistem creator outreach yapmıyor, AI-UGC üretiyor → `mt-creative-yonetmeni` | — |
...
| "Creative üret" / "reklam görseli" | Paid creative (brand veya AI-UGC tarz) → `mt-creative-yonetmeni` | App Store screenshot → `mt-aso-uzmani` |
| "Video üret" | Paid reklam videosu (her tarz) → `mt-creative-yonetmeni` | — |
| "Paywall hero" / "paywall görsel" | App içi banner / paid ad asset → `mt-creative-yonetmeni` | Paywall UI tasarımı (RevenueCat builder) → bu sistem dışı (kullanıcının kendi işi) |
| "Hook library" / "viral hook" / "trend araştır" | Trend research → `mt-creative-yonetmeni` Mod E (paid creative besler) | — |
| "Target CPI" / "target CPA" / "payback period" / "hedef ROAS" | KPI hedef belirleme → `mt-strateji-uzmani` | Mevcut CPI/CPA okuma + kararı → `mt-kampanya-analisti` |
| "Bütçe rebalance" / "kanaldan al kanala ver" | Allocation revize → `mt-strateji-uzmani` | Geçen ay neye gitti (veri) → `mt-kampanya-analisti` |
| "Aylık plan" / "ay planı" / "Haziran planı" | Plan üretimi → `mt-strateji-uzmani` | Haftalık rapor / takip → `mt-kampanya-analisti` |
| "Allocation" | Bütçe planı (ileriye dönük) → `mt-strateji-uzmani` | Performans analiz (geriye dönük) → `mt-kampanya-analisti` |
| "Shotstack" / "FLUX" / "Kling" / "Ideogram" / "Nano Banana" / "Seedance" | Üretim tarafı (fal.ai/Shotstack model adı) → `mt-creative-yonetmeni` | API token / kurulum → `mt-entegrasyon-kurucu` |
| "Bulk varyant" / "A/B set" | Çok varyant üretimi → `mt-creative-yonetmeni` | Hangi varyant kazandı (veri analiz) → `mt-kampanya-analisti` |
...
| `mt-creative-yonetmeni` | 1. "Reels reklamı için video üret" → ✅ tetiklenmeli<br>2. "5 image varyantı çıkar, A/B test için" → ✅ tetiklenmeli<br>3. "Hook copy'sini yaz, problem-first" → ✅ tetiklenmeli<br>4. "Mevcut creative'in revizyonu, logo büyüsün" → ✅ tetiklenmeli<br>5. "Shotstack ile outro ekle" → ✅ tetiklenmeli<br>6. "Paywall hero görseli üret" → ✅ tetiklenmeli (app içi banner)<br>7. "10 text overlay varyantı bulk render" → ✅ tetiklenmeli<br>8. "Brand video + caption + logo" → ✅ tetiklenmeli<br>9. "Bu hafta TikTok'ta hangi trend var?" → ✅ tetiklenmeli (Mod E — Trend Research)<br>10. "TikTok için UGC creator bulmama yardım et" → ❌ tetiklenmemeli (bu sistem dışı — creator outreach yapmıyoruz)<br>11. "App Store screenshot tasarla" → ❌ tetiklenmemeli (`mt-aso-uzmani`)<br>12. "Meta'da kampanya kur" → ❌ tetiklenmemeli (`mt-meta-ads-uzmani`)<br>13. "Hangi creative scale edilmeli?" → ❌ tetiklenmemeli (`mt-kampanya-analisti`)<br>14. "Paywall UI tasarla" → ❌ tetiklenmemeli (bu sistem dışı)<br>15. "fal.ai için yeni API key" → ❌ tetiklenmemeli (`mt-entegrasyon-kurucu`)<br>16. "Hangi kanaldan başlamalıyım?" → ❌ tetiklenmemeli (`mt-paid-ua-uzmani`) |
| `mt-strateji-uzmani` | 1. "Bu ay $500'ümü nasıl bölmeli?" → ✅ tetiklenmeli<br>2. "Haziran ayı planı yazalım, hedef D30 ROAS 1.2" → ✅ tetiklenmeli<br>3. "Target CPI ne olmalı, LTV $30" → ✅ tetiklenmeli<br>4. "Üç aylık plan hazırla" → ✅ tetiklenmeli<br>5. "Hedef 100 install için bütçe?" → ✅ tetiklenmeli<br>6. "Bütçe rebalance, TikTok'tan ASA'ya kaydır" → ✅ tetiklenmeli<br>7. "Aylık marketing planı yapalım" → ✅ tetiklenmeli<br>8. "Hangi kanaldan başlamalıyım?" → ❌ tetiklenmemeli (`mt-paid-ua-uzmani`)<br>9. "Geçen hafta kampanyalar nasıl gitti?" → ❌ tetiklenmemeli (`mt-kampanya-analisti`)<br>10. "ROAS düşük scale edebilir miyim?" → ❌ tetiklenmemeli (`mt-kampanya-analisti`)<br>11. "Meta'da AAC nasıl kurulur?" → ❌ tetiklenmemeli (`mt-meta-ads-uzmani`)<br>12. "ROAS ne demek?" → ❌ tetiklenmemeli (`mt-marketing-tutor`)<br>13. "Creative üret" → ❌ tetiklenmemeli (`mt-creative-yonetmeni`) |
| `mt-kampanya-analisti` | 1. "Bu hafta kampanyalar nasıl?" → ✅ tetiklenmeli<br>2. "ROAS düşük neden?" → ✅ tetiklenmeli<br>3. "Şu kampanyayı kessem mi?" → ✅ tetiklenmeli<br>4. "Scale edebilir miyim?" → ✅ tetiklenmeli<br>5. "Mayıs cohort analizi" → ✅ tetiklenmeli<br>6. "Mayıs ay sonu retrospektif" → ✅ tetiklenmeli<br>7. "CPI yükseldi anomali" → ✅ tetiklenmeli<br>8. "Aylık plan yazalım" → ❌ tetiklenmemeli (`mt-strateji-uzmani`)<br>9. "Hangi kanaldan başla" → ❌ tetiklenmemeli (`mt-paid-ua-uzmani`)<br>10. "Meta'da yeni kampanya kur" → ❌ tetiklenmemeli (`mt-meta-ads-uzmani`)<br>11. "Creative üret" → ❌ tetiklenmemeli (`mt-creative-yonetmeni`)<br>12. "ROAS ne demek?" → ❌ tetiklenmemeli (`mt-marketing-tutor`) |
| `mt-rakip-arastirmaci` | 1. "Streaks app'i analiz et" → ✅ tetiklenmeli<br>2. "Habit tracker kategorisinde rakipler" → ✅ tetiklenmeli<br>3. "Calm paywall'una bak" → ✅ tetiklenmeli<br>4. "TikTok'ta finance app reklamları" → ✅ tetiklenmeli<br>5. "5 fitness app pricing karşılaştır" → ✅ tetiklenmeli<br>6. "Habitica son güncellemede ne değiştirmiş" → ✅ tetiklenmeli<br>7. "Kendi ASO'mu denetle" → ❌ tetiklenmemeli (`mt-aso-uzmani`)<br>8. "Rakip creative'ini taklit et üret" → ❌ tetiklenmemeli (`mt-creative-yonetmeni`)<br>9. "Pricing değişimi" → ❌ tetiklenmemeli (`mt-strateji-uzmani`) |
| `mt-aso-uzmani` | 1. "ASO denetimi yap" → ✅ tetiklenmeli<br>2. "Keyword optimize" → ✅ tetiklenmeli<br>3. "Screenshot başlıkları öner" → ✅ tetiklenmeli<br>4. "CPP yaratalım" → ✅ tetiklenmeli<br>5. "Negatif review reply stratejisi" → ✅ tetiklenmeli<br>6. "App Store ikon önerisi" → ✅ tetiklenmeli<br>7. "ASA kampanyası kur" → ❌ tetiklenmemeli (`mt-apple-search-ads-uzmani`)<br>8. "Paid reklam görseli üret" → ❌ tetiklenmemeli (`mt-creative-yonetmeni`)<br>9. "Rakip ASO" → ❌ tetiklenmemeli (`mt-rakip-arastirmaci`)<br>10. "Keyword nedir?" → ❌ tetiklenmemeli (`mt-marketing-tutor`) |
| `mt-entegrasyon-kurucu` | 1. "Meta Marketing API kurulumu" → ✅ tetiklenmeli<br>2. "Google Ads API token al" → ✅ tetiklenmeli<br>3. "Yeni MCP ekleyelim" → ✅ tetiklenmeli<br>4. "Token expired" → ✅ tetiklenmeli<br>5. "401 alıyorum debug" → ✅ tetiklenmeli<br>6. "Firebase event handoff MD" → ✅ tetiklenmeli<br>7. "Meta'da hesap aç" → ❌ tetiklenmemeli (`mt-hesap-kurulum-rehberi`)<br>8. "Meta'da kampanya kur" → ❌ tetiklenmemeli (`mt-meta-ads-uzmani`)<br>9. "API nedir?" → ❌ tetiklenmemeli (`mt-marketing-tutor`)<br>10. "Performans analizi" → ❌ tetiklenmemeli (`mt-kampanya-analisti`) |
